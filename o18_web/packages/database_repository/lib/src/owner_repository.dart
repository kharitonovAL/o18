...
class OwnerRepository {
  ...
  Future<List<model.Owner>> getOwnerList() async {
    ...
    if (q.results != null) {
      final list =
          q.results!.map((dynamic owner) => owner as model.Owner).toList();
      return list;
    }
    return [];
  }
  ...
  Future<List<model.Owner>> loadOwnerListForAccount({
    ...
    final QueryBuilder accountQuery =
        QueryBuilder<model.Account>(model.Account());
    accountQuery
      ..setLimit(queryLimit)
      ..whereEqualTo(model.Account.keyAccountNumber, accountNumber);
    final aq = await accountQuery.query();

    final accountList = aq.results == null
        ? <model.Account>[]
        : aq.results!.map((dynamic acc) => acc as model.Account).toList();
    ...
    final ownerList = oq.results == null
        ? <model.Owner>[]
        : oq.results!.map((dynamic owner) => owner as model.Owner).toList();
  ...
  Future<String> getOwnerAddress({
    ...
    final account = accountList
        .firstWhere((account) => account.objectId == owner.accountId);
    ...
    final house =
        houseList.firstWhere((house) => house.objectId == flat.houseId);

    final address =
        'ул.${house.street}, д.${house.houseNumber}, кв.${flat.flatNumber}';
    return address;
  }
}
