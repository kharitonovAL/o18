...
import 'package:o18_client/theme/style/app_fonts.dart';
...
class _RequestListItemState extends State<RequestListItem> {
  ...
  Color _setFontColor() {
    if (widget.request.responseDate != null) {
      if (widget.request.status == RS.received && DateTime.now().isAfter(widget.request.responseDate!)) {
        return AppColors.red;
      }
      if (widget.request.status == RS.inProgress && DateTime.now().isAfter(widget.request.responseDate!)) {
        return AppColors.red;
      }
      ...
      if (widget.request.status == RS.passedToTheMaster && DateTime.now().isAfter(widget.request.responseDate!)) {
  ...
  FontWeight _setFontWeight() =>
      widget.request.wasSeen != null && widget.request.wasSeen == false ? FontWeight.bold : FontWeight.normal;
}
